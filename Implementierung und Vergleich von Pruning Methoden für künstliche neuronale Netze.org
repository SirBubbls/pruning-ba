...
Jedoch muss auch erwähnt werden, dass das Projekt auch ohne weitere Entwicklung für \ac{ML} Projekte definitiv benutzbar ist.
...
Zudem wäre es sehr interessant, weitere Tests des Pruning Frameworks auf verschiedenen evtl. nicht standard \ac{ANN} Architekturen durchzuführen.
Eventuell können so neue Erkenntnisse über das Pruning Potential verschiedener \ac{ANN} Architekturen gesammelt werden.
...
Eventuell lässt sich auch durch spezialisierte Software Lösungen ein ähnlicher Leistungsanstieg verzeichnen.
